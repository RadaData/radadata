...
function getProxy() {
  ...
  $db->prepare("UPDATE proxies SET last_used = :time WHERE `proxy` = :proxy;")->execute(array(':time' => time(), ':proxy' => $proxy));
  $db->commit();
...
function releaseProxy() {
  global $proxy;
  db('misc')->prepare("UPDATE proxies SET last_used = :time WHERE `proxy` = :proxy;")->execute(array(':time' => 0, ':proxy' => $proxy));
  $proxy = null;
}