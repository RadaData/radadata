...
class CheckCommand extends Console\Command\Command
{
    ...
    protected function execute(Console\Input\InputInterface $input, Console\Output\OutputInterface $output)
    {
        ...
        $i = 1;
        Law::where('status', '<', Law::SAVED)->orderBy('id')->chunk(200, function($laws) use ($fix, $law_dir, &$i, &$result_count, &$nd_orphaned_dirs, &$d_no_files, &$d_fake_content, &$d_unknown_text_true_content, &$d_unknown_text_no_text) {
            foreach ($laws as $law) {
